...
function abrirModalAdicionar() {
    ...
    // Buscar EDPs
    $.get('/circuitos/edps/api').done(function(edps) {
                ...
                <div class="row g-3">
                    <div class="col-12">
                        <div class="card shadow-sm">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    <label class="form-label"><strong>EDP:</strong> <span class="text-danger">*</span></label>
                                    <select class="form-control" name="id_edp" id="edp-select" required>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" name="id_subestacao" id="subestacao-select" disabled required>
                                        <option value="">Selecione EDP primeiro...</option>
                                    </select>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label"><strong>Circuito:</strong> <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" name="circuito" id="campo-circuito" disabled required placeholder="Selecione EDP e Subestação">
        ...
        // Restante do código (handlers de change, etc.) permanece igual...
        $('#edp-select').on('change', function() {
            let edpId = $(this).val();
            if (!edpId) {
                $('#subestacao-select').html('<option value="">Selecione EDP primeiro...</option>').prop('disabled', true);
                prepararCampoCircuito();
                return;
            }

            $('#subestacao-select').html('<option>Carregando...</option>').prop('disabled', true);

            $.get('/circuitos/subestacoes/api/' + edpId).done(function(subs) {
                if (subs.length === 0) {
                    $('#subestacao-select').html('<option value="">Nenhuma subestação disponível</option>').prop('disabled', true);
                } else {
                    let options = '<option value="">Selecione...</option>' + subs.map(sub =>
                        `<option value="${sub.id}" data-sigla="${sub.sigla}">${sub.nome}</option>`
                    ).join('');
                    $('#subestacao-select').html(options).prop('disabled', false);
                }
                prepararCampoCircuito();
            }).fail(function() {
                $('#subestacao-select').html('<option value="">Erro ao buscar subestações</option>').prop('disabled', true);
                prepararCampoCircuito();
            });
        });
        ...
        $modalBody.on('input', '#campo-circuito', function() {
            let edpText = $("#edp-select option:selected").text().trim();
            let $subOption = $("#subestacao-select option:selected");
            let sigla = $subOption.data('sigla');

            if (edpText === 'ES') {
                if (!this.value.startsWith(sigla)) {
                    this.value = sigla;
                    this.setSelectionRange(sigla.length, sigla.length);
                }
            } else if (edpText === 'SP') {
                let prefixo = `R${sigla} `;
        ...
        function prepararCampoCircuito() {
            ...
            let $subOption = $("#subestacao-select option:selected");
            let sigla = $subOption.data('sigla');
            let $circuito = $('#campo-circuito');
            let $dica = $('#dica-circuito');

            if (!$("#edp-select").val() || !$("#subestacao-select").val()) {
                $circuito.prop('disabled', true).val('');
                $dica.text('Selecione EDP e Subestação antes de preencher o Circuito.');
                return;
            }
            ...
            if (edpText === 'ES') {
                ...
                $dica.text(`Formato: ${sigla}NN (exemplo: ${sigla}03)`);
                setTimeout(() => $circuito[0].setSelectionRange(sigla.length, sigla.length), 0);
            } else if (edpText === 'SP') {
                let prefixo = `R${sigla} `;
                $circuito.prop('disabled', false)
                    .val(prefixo)
                    .attr('maxlength', prefixo.length + 2);
                $dica.text(`Formato: ${prefixo}NN (exemplo: ${prefixo}03)`);
                setTimeout(() => $circuito[0].setSelectionRange(prefixo.length, prefixo.length), 0);
            } else {
                $circuito.prop('disabled', true).val('');
                $dica.text('Selecione EDP do tipo "ES" ou "SP".');
            }
        }
    }).fail(function() {
