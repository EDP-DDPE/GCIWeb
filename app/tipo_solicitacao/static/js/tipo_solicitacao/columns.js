...
export function saveTableSettings() {
    ...
    try {
        localStorage.setItem("tipoSolicitacaoTableSettings", JSON.stringify(settings));
    } catch (e) {
...
export function loadTableSettings() {
    try {
        const settings = JSON.parse(localStorage.getItem("tipoSolicitacaoTableSettings") || "{}");
...
// Expor para os onclick do HTML (botão "Colunas", "Todas", "Nenhuma")
